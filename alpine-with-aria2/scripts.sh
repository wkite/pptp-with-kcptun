...
#!/bin/sh
#set -ex
SAVE_DIR=/var/www/html/media/Downloads
SRC_URL="$1"
SAVE_NAME="$2"
if [[ -z ${SRC_URL} ]]; then
...
nohup axel -n 8 ${SAVE_PATH} -U "Mozilla/5.0 (Windows NT 10.0; Win64; x64) \AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.100 Safari/537.36 " \
-H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3" \
-H "Accept-Encoding: gzip, deflate " -H "Accept-Language: zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7" "${SRC_URL}" > ${LOG_PATH}/nohup.out 2>&1 &
echo -e "axel ${SRC_URL} started, \nlog could be found in ${LOG_DIR}/nohup.out"
EOFF
...
TARGET=$1
SAVE_DIR=/var/www/html/media/Downloads
echo "# du -sm ${SAVE_DIR}/*"${TARGET}"*" && du -sm ${SAVE_DIR}/*"${TARGET}"*
echo -e "\n# rm -rf ${SAVE_DIR}/*${TARGET}*" && rm -rf ${SAVE_DIR}/*"${TARGET}"* && echo "${SAVE_DIR}/*${TARGET}* has been cleaned up."
echo -e "\n# du -sm *" && du -sm ${SAVE_DIR}/* || echo 'None file.'
EOF
