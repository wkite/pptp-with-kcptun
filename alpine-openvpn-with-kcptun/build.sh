...
docker run -d --name $NAME-$PORT --init --restart=always --cap-add NET_ADMIN --device=/dev/net/tun -e USERNAME=admin -e PASSWORD=$PASSWORD -e KCP_ENCRYPT=none -e KCP_PASSWORD=none-encrypt -p $PORT:29900/udp $NAME:$PORT
